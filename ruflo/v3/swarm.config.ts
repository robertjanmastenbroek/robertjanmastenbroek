...
import type { SwarmConfig } from './src/shared/types';

// Local type aliases for types not exported from shared/types
type TopologyType = 'hierarchical' | 'mesh' | 'simple' | 'adaptive';
type LoadBalancingStrategy = 'round-robin' | 'capability-match' | 'least-loaded' | string;
type AgentDomain = 'security' | 'core' | 'integration' | 'quality' | 'performance' | 'deployment' | string;
type PhaseId = 'phase-1-foundation' | 'phase-2-core' | 'phase-3-integration' | 'phase-4-release' | string;

interface PerformanceTargets {
  flashAttentionSpeedup: string;
  heirarchicalSearchSpeedup: string;
...
const V3_PERFORMANCE_TARGETS: PerformanceTargets = {
  ...
  heirarchicalSearchSpeedup: '150x-12500x',
  tokenReduction: '40%',
  latencyTarget: '<50ms p99',
};
