#!/usr/bin/env python3
"""
RJM Research Agent — Claude CLI wrapper for rjm-research.

Reads the rjm-research scheduled task SKILL.md and runs it via Claude CLI
...
Claude Code scheduler — this wrapper lets you trigger it manually:
  ...
  python3 research_agent.py            # research all pending contacts
  python3 research_agent.py --dry-run  # print the skill prompt, don't run

Called by master_agent.py run research and rjm.py run research.
"""
...
# ─── Skill path ───────────────────────────────────────────────────────────────
SKILL_PATH   = Path.home() / ".claude/scheduled-tasks/rjm-research/SKILL.md"
...
def main():
    parser = argparse.ArgumentParser(description="RJM Research Agent")
    ...
    if not SKILL_PATH.exists():
        ...
        print("   Install or re-sync the rjm-research scheduled task.")
        sys.exit(1)

    skill_content = SKILL_PATH.read_text(encoding="utf-8")

    if args.dry_run:
        print("─── rjm-research SKILL.md ───────────────────────────────")
        print(skill_content)
        print("────────────────────────────────────────────────────────")
    ...
    print(f"→ Running rjm-research via Claude CLI…\n")
