...
async function getUpcomingEvents() {
  ...
  const events = await sql`
    SELECT
      ...
      e.image_url, e.maps_url, e.status,
...
async function seedEvent({ slug, title, location, location_detail, event_date, event_time, description, ticket_limit, image_url, maps_url }) {
  const sql = getSql();
  await sql`
    INSERT INTO events (slug, title, location, location_detail, event_date, event_time, description, ticket_limit, image_url, maps_url)
    VALUES (${slug}, ${title}, ${location}, ${location_detail || null}, ${event_date}, ${event_time || 'Sunset'}, ${description || null}, ${ticket_limit || 50}, ${image_url || null}, ${maps_url || null})
