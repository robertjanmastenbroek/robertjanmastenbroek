...
async function createRegistration({ id, firstName, lastName, email, phone, phoneVerified, amount, quantity, week, stripeSessionId, eventId }) {
  const sql = getSql();
  const status = amount === 0 ? 'confirmed' : 'pending';
  const qty = Math.max(1, quantity || 1);
  const confirmationCode = await generateConfirmationCode();
  await sql`
    INSERT INTO holy_rave_registrations (id, first_name, last_name, email, phone, phone_verified, amount_cents, quantity, status, week, stripe_session_id, event_id, confirmation_code)
    VALUES (${id}, ${firstName}, ${lastName}, ${email}, ${phone || null}, ${!!phoneVerified}, ${amount}, ${qty}, ${status}, ${week || null}, ${stripeSessionId || null}, ${eventId || null}, ${confirmationCode})
  `;
...
async function getPendingRegistrations(minutesOld = 20) {
  ...
  return await sql`
    ...
    WHERE hr.status = 'pending'
      ...
      AND hr.created_at < NOW() - (${minutesOld} || ' minutes')::INTERVAL
...
async function markReminderSent(id) {
  const sql = getSql();
  await sql`
    UPDATE holy_rave_registrations SET reminder_sent = TRUE WHERE id = ${id}
  `;
}
