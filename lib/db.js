...
async function getUpcomingEvents() {
  ...
  const events = await sql`
    SELECT
      ...
      e.image_url, e.maps_url, e.status, e.pricing_model, e.ticket_price_cents,
...
async function seedEvent({ slug, title, location, location_detail, event_date, event_time, description, ticket_limit, image_url, maps_url, pricing_model, ticket_price_cents }) {
  const sql = getSql();
  const pm = pricing_model || 'pay_what_you_want';
  const price = pm === 'fixed' ? (ticket_price_cents || 0) : null;
  await sql`
    INSERT INTO events (slug, title, location, location_detail, event_date, event_time, description, ticket_limit, image_url, maps_url, pricing_model, ticket_price_cents)
    VALUES (${slug}, ${title}, ${location}, ${location_detail || null}, ${event_date}, ${event_time || 'Sunset'}, ${description || null}, ${ticket_limit || 50}, ${image_url || null}, ${maps_url || null}, ${pm}, ${price})
...
async function createRegistration({ id, firstName, lastName, email, phone, phoneVerified, amount, quantity, week, stripeSessionId, eventId, utmSource, utmMedium, utmCampaign, smsStatus, emailOnly }) {
  const sql = getSql();
  ...
  await sql`
    INSERT INTO holy_rave_registrations (id, first_name, last_name, email, phone, phone_verified, amount_cents, quantity, status, week, stripe_session_id, event_id, confirmation_code, payment_token, utm_source, utm_medium, utm_campaign, sms_status, email_only)
    VALUES (${id}, ${firstName}, ${lastName}, ${email}, ${phone || null}, ${!!phoneVerified}, ${amount}, ${qty}, ${status}, ${week || null}, ${stripeSessionId || null}, ${eventId || null}, ${confirmationCode}, ${paymentToken}, ${utmSource || null}, ${utmMedium || null}, ${utmCampaign || null}, ${smsStatus || null}, ${!!emailOnly})
    ON CONFLICT (id) DO UPDATE SET
      ...
      status = CASE WHEN holy_rave_registrations.status = 'confirmed' THEN 'confirmed' ELSE EXCLUDED.status END,
      stripe_session_id = COALESCE(EXCLUDED.stripe_session_id, holy_rave_registrations.stripe_session_id),
      event_id = COALESCE(EXCLUDED.event_id, holy_rave_registrations.event_id),
      utm_source = COALESCE(EXCLUDED.utm_source, holy_rave_registrations.utm_source),
      utm_medium = COALESCE(EXCLUDED.utm_medium, holy_rave_registrations.utm_medium),
      utm_campaign = COALESCE(EXCLUDED.utm_campaign, holy_rave_registrations.utm_campaign),
...
async function getRegistrationByPaymentToken(token) {
  const sql = getSql();
  const rows = await sql`
    SELECT hr.*, e.slug AS event_slug, e.title AS event_title, e.event_date, e.event_time, e.location,
           e.ticket_limit, e.image_url, e.pricing_model, e.ticket_price_cents
    FROM holy_rave_registrations hr
    LEFT JOIN events e ON e.id = hr.event_id
    WHERE hr.payment_token = ${token}
  `;
  return rows[0] || null;
}

async function getRegistrationByStripeSession(stripeSessionId) {
  const sql = getSql();
  const rows = await sql`
    SELECT * FROM holy_rave_registrations WHERE stripe_session_id = ${stripeSessionId}
  `;
  return rows[0] || null;
}
...
async function isDuplicateEmailForEvent(eventId, email) {
  ...
  // First, expire stale pending registrations for this email + event
  await sql`
    UPDATE holy_rave_registrations
    SET status = 'expired'
    WHERE event_id = ${eventId}
  ...
  // Then check for any active duplicate
  const [row] = await sql`
    SELECT status FROM holy_rave_registrations
    WHERE event_id = ${eventId}
      AND email = ${email}
...
async function getRegistrationsByEventId(eventId) {
  const sql = getSql();
  return await sql`
    SELECT id, first_name, last_name, email, phone, amount_cents, status,
      confirmation_code, created_at, confirmed_at, stripe_session_id
    FROM holy_rave_registrations
    WHERE event_id = ${eventId}
    ORDER BY created_at DESC
    LIMIT 100
...
async function cancelRegistration(id) {
  ...
  const [row] = await sql`
    ...
    RETURNING id
  `;
  if (row) console.log(`[db] Registration cancelled: ${row.id}`);
  return row || null;
}
...
// ─── Abandoned registration reminder helpers ─────────────────────────────
async function getPendingRegistrations(minutesOld = 20) {
  const sql = getSql();
  // Returns pending registrations that need their NEXT email in the sequence.
  // Step 0 (no email sent): older than 15 minutes → send first email
  // Step 1 (first sent): older than 2 hours → send second email
  // Step 2 (second sent): older than 24 hours → send third email
  // Step 3+ (all sent): never returned
  return await sql`
    SELECT hr.id, hr.first_name, hr.last_name, hr.email, hr.phone, hr.event_id,
           hr.payment_token, hr.confirmation_code,
           e.title AS event_title, e.event_date, e.event_time, e.location, e.slug,
           e.pricing_model, e.ticket_price_cents,
           hr.created_at, hr.email_sequence_step
    FROM holy_rave_registrations hr
    LEFT JOIN events e ON e.id = hr.event_id
    WHERE hr.status = 'pending'
      AND (
        (hr.email_sequence_step = 0 AND hr.created_at < NOW() - INTERVAL '15 minutes')
        OR
        (hr.email_sequence_step = 1 AND hr.created_at < NOW() - INTERVAL '2 hours')
        OR
        (hr.email_sequence_step = 2 AND hr.created_at < NOW() - INTERVAL '24 hours')
      )
    ORDER BY hr.created_at ASC
  `;
}

async function markEmailStep(id, step) {
  const sql = getSql();
  await sql`
    UPDATE holy_rave_registrations
    SET email_sequence_step = ${step},
        reminder_sent = TRUE
    WHERE id = ${id}
  `;
}
...
// ─── Holy Rave Hub Background ──────────────────────────────────────────────

async function getHubBackground() {
  return await getSiteSetting('holy_rave_hub_background');
}

async function setHubBackground(url) {
  const cleaned = url && url.trim() ? url.trim() : null;
  await setSiteSetting('holy_rave_hub_background', cleaned);
}
