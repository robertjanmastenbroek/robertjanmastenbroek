...
async function createRegistration({ id, firstName, lastName, email, phone, phoneVerified, amount, quantity, week, stripeSessionId, eventId, utmSource, utmMedium, utmCampaign, smsStatus }) {
  const sql = getSql();
  const status = amount === 0 ? 'confirmed' : 'pending';
  const qty = Math.max(1, quantity || 1);
  const confirmationCode = await generateConfirmationCode();
  await sql`
    INSERT INTO holy_rave_registrations (id, first_name, last_name, email, phone, phone_verified, amount_cents, quantity, status, week, stripe_session_id, event_id, confirmation_code, utm_source, utm_medium, utm_campaign, sms_status)
    VALUES (${id}, ${firstName}, ${lastName}, ${email}, ${phone || null}, ${!!phoneVerified}, ${amount}, ${qty}, ${status}, ${week || null}, ${stripeSessionId || null}, ${eventId || null}, ${confirmationCode}, ${utmSource || null}, ${utmMedium || null}, ${utmCampaign || null}, ${smsStatus || null})
  `;
