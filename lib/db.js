...
async function createRegistration({ id, firstName, lastName, email, amount, quantity, week, stripeSessionId, eventId }) {
  const sql = getSql();
  const status = amount === 0 ? 'confirmed' : 'pending';
  const qty = Math.max(1, quantity || 1);
  await sql`
    INSERT INTO holy_rave_registrations (id, first_name, last_name, email, amount_cents, quantity, status, week, stripe_session_id, event_id)
    VALUES (${id}, ${firstName}, ${lastName}, ${email}, ${amount}, ${qty}, ${status}, ${week || null}, ${stripeSessionId || null}, ${eventId || null})
  `;
