...
// Lazy-initialize the Neon client so the server starts even without DATABASE_URL.
// If DATABASE_URL is not set, all DB operations will throw a clear error.
function getSql() {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is not set — database not configured.');
  }
  if (!getSql._instance) {
    getSql._instance = neon(process.env.DATABASE_URL);
  }
  return getSql._instance;
}
...
async function createRegistration({ id, firstName, lastName, email, amount, week, stripeSessionId }) {
  const sql = getSql();
  const status = amount === 0 ? 'confirmed' : 'pending';
  await sql`
    INSERT INTO holy_rave_registrations (id, first_name, last_name, email, amount_cents, status, week, stripe_session_id)
    VALUES (${id}, ${firstName}, ${lastName}, ${email}, ${amount}, ${status}, ${week}, ${stripeSessionId || null})
  `;
...
async function confirmRegistration(id, week) {
  const sql = getSql();
  // Only confirm if under capacity
  const [countRow] = await sql`
    SELECT COUNT(*)::int AS count FROM holy_rave_registrations
    WHERE week = ${week} AND status = 'confirmed'
  `;
  const TICKETS_MAX = 50;
  if (countRow.count >= TICKETS_MAX) {
    console.warn(`[db] Overbook blocked: ${id} — week ${week} at ${countRow.count}/${TICKETS_MAX}`);
    return false;
  }
  await sql`
...
async function getWeekStats(week) {
  ...
  return {
    ...
    total: TICKETS_MAX,
    sold: row.confirmed,
    remaining: Math.max(0, TICKETS_MAX - row.confirmed),
  };
}
