...
# Veo 3.1 first-last-frame-to-video — Google's premium keyframe-chain engine.
# 2.4× the cost of Kling O3 (~$0.20/s vs $0.084/s at 1080p). Max duration 8s
# per clip (vs Kling's 10s). Audio generated by default; must pass
# `generate_audio: false` since we overlay the track via Shotstack.
#
# ─── Decision log (keep this — don't re-test without reading first) ──────────
#
# Tested 2026-04-22 on Halleluyah (4 keyframes × 8s chain, 32s total loop,
# ~$6.40 spend, ~10 min wall-clock). Verdict:
#   Quality — marginal lift on face consistency + particle motion vs Kling
#   O3. Not enough to justify 2.4× cost on a production pipeline that
#   renders ~9-keyframe chains per track.
#   Safety — 1 of 4 Iron-Age-Hebrew ceremony morphs tripped Veo's policy
#   filter ("mouth open mid-shout" + "hands lifted skyward" + "chanting
#   elder" triggered). `auto_fix=True` retry worked but silently rewrites
#   the prompt — not acceptable for brand-controlled content.
#   Duration — 8s cap forces awkward chain geometry (32s instead of clean
#   30s at 3×10s).
#
# Also evaluated same day: Seedance 2.0 (bytedance/seedance-2.0/*). Tops
# 2026 composite quality leaderboards (VideoGen-Eval) but priced at
# $0.3024/s @ 720p → 3.6× Kling, and ~$0.68/s @ 1080p → ~8× Kling.
# Rejected on cost before any test spend.
#
# Kling O3 Standard remains the default morph engine. Revisit this endpoint
# only if:
#   (a) Veo 3.1 pricing drops ≥50%, or
#   (b) a new face/motion benchmark shows clear generational lift, or
#   (c) Kling O3 starts failing on face consistency at a blocking rate.
#
# No opt-in Veo wiring in the code — removed 2026-04-22 after the A/B.
# Endpoint kept here only so future tests start from the correct slug.
